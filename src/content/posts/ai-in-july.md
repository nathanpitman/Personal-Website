...
One key learning here for me has been integrating tools like [Langfuse](https://langfuse.com) into the architecture, so we can observe how the LLM interprets learner input and where it might be drifting from the content we've grounded it in and the guardrails we've established. While very early versions of this "worked", even with AI the devil is in the details, particularly in the need to establish deterministic outcomes where you want to be able to confidently prove you've assessed someones understanding.

## Enricher: turning three form fields into a full company profile

At [iHasco](https://ihasco.co.uk) we needed a way to validate some of the prospect data attributes we see coming from our marketing website lead forms. Initially this started as a one off script built with [Claude Code](https://claude.ai/referral/SXnxTTUikA) to ingest, review and enrich prospect data to understand if we're under representing the value of particular leads.
